...
def flip_stereo_in_smiles(smiles, flip_indices):
    ...
    assert len(matches) >= max(flip_indices, default=-1) + 1, ""
    smiles_new = smiles
    offset = 0
...
def _convert_graph_to_smiles(coords, symbols, edges, image=None, debug=False):
    ...
    # Safety check for edges dimension
    if len(edges) != n or (len(edges) > 0 and len(edges[0]) != n):
        # if debug:
        #     print(f"Error: Mismatch between symbols length ({n}) and edges dimensions ({len(edges)}x{len(edges[0]) if edges else 0})")
